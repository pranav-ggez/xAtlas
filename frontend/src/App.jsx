...
import EmailBreachChecker from './components/EmailBreachChecker.jsx'
...
function App() {
  ...
  useEffect(() => {
    ...
    const types = ["DDoS", "SQL Injection", "Brute Force", "Malware C2", "Zero-Day Exploit", "Ransomware", "Phishing", "APT Activity"]
    ...
    const generateHistoricalBaseline = () => {
      ...
      for (let i = 0; i < 500; i++) {
        ...
        let sourceIdx
        const r = Math.random()
        if (r < 0.35) sourceIdx = 2
        else if (r < 0.60) sourceIdx = 1
        else if (r < 0.75) sourceIdx = 7
        else if (r < 0.85) sourceIdx = 9
        else sourceIdx = Math.floor(Math.random() * cities.length)
        let targetIdx
        const t = Math.random()
        if (t < 0.40) targetIdx = 0
        else if (t < 0.60) targetIdx = 4
        else if (t < 0.75) targetIdx = 6
    ...
    const fetchLiveThreats = async () => {
      try {
        const res = await fetch('/api/security/threatfox/latest?limit=10')
        if (!res.ok) throw new Error('Fetch failed')
        const data = await res.json()
        if (data.query_status !== 'ok') return
        const threatMap = {
          'malware-c2': 'Malware C2', 'phishing': 'Phishing', 'botnet': 'DDoS',
          'ransomware': 'Ransomware', 'trojan': 'Zero-Day Exploit',
          'spam': 'Brute Force', 'default': 'APT Activity'
        }
        const newAttacks = data.data.map(item => {
          ...
          return createAttack(sourceIdx, targetIdx, attackType,
            ['Ransomware', 'Zero-Day Exploit'].includes(attackType) ? 'Critical' : 'High',
            item.date || Date.now())
        })
        setAttacks(prev => [...newAttacks, ...prev].slice(0, 100))
      } catch (e) {}
    }

    setAttacks(generateHistoricalBaseline())
    fetchLiveThreats()
    const interval = setInterval(fetchLiveThreats, 15000)
  ...
  const stats = {
    ...
    topAttacker: attacks.length > 0
      ? Object.entries(attacks.reduce((acc, a) => { acc[a.source_country] = (acc[a.source_country] || 0) + 1; return acc }, {}))
          .sort((a, b) => b[1] - a[1])[0]?.[0] || 'N/A' : 'N/A',
    topTarget: attacks.length > 0
      ? Object.entries(attacks.reduce((acc, a) => { acc[a.target_country] = (acc[a.target_country] || 0) + 1; return acc }, {}))
          .sort((a, b) => b[1] - a[1])[0]?.[0] || 'N/A' : 'N/A'
  }
  ...
  const BASE_TOOLS = [
    {
      key: 'oss',
      title: '🔍 OSS Alternative Finder',
      color: '#10b981',
      border: '#1a3a2a',
      bg: 'rgba(16,185,129,0.05)',
      component: <SearchPanel onSelectTool={setSelectedTool} />
    },
    {
      key: 'password',
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        border: '1px solid #10b981', borderBottom: '2px solid #10b981',
        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        padding: '0 15px', zIndex: 100,
        boxShadow: '0 0 20px rgba(16, 185, 129, 0.3)', boxSizing: 'border-box'
      }}>
        <div style={{ overflow: 'hidden', flexShrink: 0 }}>
          <h1 style={{ margin: 0, fontSize: '1.3rem', color: '#fff', fontWeight: 'bold', letterSpacing: '1px', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ display: 'flex', gap: '4px', alignItems: 'center', flex: 1, justifyContent: 'center' }}>
          {navSections.map(section => (
            <button key={section.key} onClick={() => scrollToSection(section.ref, section.key)}
              style={{
              ...
              onMouseEnter={(e) => { if (activeSection !== section.key) { e.currentTarget.style.borderColor = '#10b981'; e.currentTarget.style.color = '#10b981' } }}
              onMouseLeave={(e) => { if (activeSection !== section.key) { e.currentTarget.style.borderColor = '#333'; e.currentTarget.style.color = '#888' } }}
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexShrink: 0 }}>
          ...
          <button onClick={togglePause} style={{
            ...
            fontSize: '0.8rem', fontWeight: 'bold', display: 'flex', alignItems: 'center',
            gap: '8px', transition: 'all 0.2s', minWidth: '140px', justifyContent: 'center'
          }}>
      ...
      <div ref={mainScrollRef} style={{
        ...
        scrollbarWidth: 'thin', scrollbarColor: '#10b981 #001100', scrollBehavior: 'smooth'
      }}>

        {/* ── SECTION 1: GLOBE ── */}
        <div ref={globeSectionRef} style={{
          position: 'relative', width: '100%',
          height: 'calc(100vh - 50px)', minHeight: '600px',
          ...
          {!isPaused && (
            <>
              ...
              <div style={{
                ...
                background: 'rgba(0, 10, 0, 0.95)', borderRight: '1px solid #10b981',
                zIndex: 90, overflow: 'hidden', transition: 'width 0.3s ease'
              }}>
                <div style={{ width: '280px', padding: '12px', height: '100%', overflowY: 'auto', overflowX: 'hidden', scrollbarWidth: 'thin', scrollbarColor: '#10b981 #001100', boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>
                  <div style={{ marginBottom: '15px' }}>
                    <h3 style={{ margin: '0 0 8px 0', color: '#10b981', fontSize: '0.8rem', textTransform: 'uppercase', borderBottom: '1px solid #333', paddingBottom: '6px' }}>📡 LAYERS</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      {[
                        ...
                        <label key={layer.key} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px', background: activeLayers[layer.key] ? 'rgba(16, 185, 129, 0.1)' : 'transparent', border: `1px solid ${activeLayers[layer.key] ? layer.color : '#333'}`, borderRadius: '4px', cursor: 'pointer', transition: 'all 0.2s' }}>
                          <input type="checkbox" checked={activeLayers[layer.key]} onChange={() => toggleLayer(layer.key)} style={{ accentColor: layer.color, width: '14px', height: '14px', cursor: 'pointer' }} />
                  ...
                  <div style={{ marginTop: '20px', borderTop: '1px solid #333', paddingTop: '15px' }}>
                    <h3 style={{ margin: '0 0 15px 0', color: '#10b981', fontSize: '0.8rem', textTransform: 'uppercase', borderBottom: '1px solid #333', paddingBottom: '6px', fontFamily: '"Press Start 2P", cursive, monospace', letterSpacing: '1px' }}>CONNECT</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                      {[
                        { href: 'https://github.com/pranav-ggez', label: 'GITHUB', bg: 'rgba(255,255,255,0.05)', border: '#333', icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" style={{ color: '#fff', flexShrink: 0 }}><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg> },
                        { href: 'https://www.linkedin.com/in/pranav-vaidya-025326255/', label: 'LINKEDIN', bg: 'rgba(10,102,194,0.15)', border: '#0a66c2', icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" style={{ color: '#0a66c2', flexShrink: 0 }}><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg> },
                        { href: 'https://discord.com', label: 'DISCORD', bg: 'rgba(88,101,242,0.15)', border: '#5865F2', icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" style={{ color: '#5865F2', flexShrink: 0 }}><path d="M20.317 4.3698a19.7913 19.7913 0 00-4.8851-1.5152.0741.0741 0 00-.0785.0371c-.211.3753-.4447.8648-.6083 1.2495-1.8447-.2762-3.68-.2762-5.4868 0-.1636-.3933-.4058-.8742-.6177-1.2495a.077.077 0 00-.0785-.037 19.7363 19.7363 0 00-4.8852 1.515.0699.0699 0 00-.0321.0277C.5334 9.0458-.319 13.5799.0992 18.0578a.0824.0824 0 00.0312.0561c2.0528 1.5076 4.0413 2.4228 5.9929 3.0294a.0777.0777 0 00.0842-.0276c.4616-.6304.8731-1.2952 1.226-1.9942a.076.076 0 00-.0416-.1057c-.6528-.2476-1.2743-.5495-1.8722-.8923a.077.077 0 01-.0076-.1277c.1258-.0943.2517-.1923.3718-.2914a.0743.0743 0 01.0776-.0105c3.9278 1.7933 8.18 1.7933 12.0614 0a.0739.0739 0 01.0785.0095c.1202.099.246.1981.3728.2924a.077.077 0 01-.0066.1276 12.2986 12.2986 0 01-1.873.8914.0766.0766 0 00-.0407.1067c.3604.698.7719 1.3628 1.225 1.9932a.076.076 0 00.0842.0286c1.961-.6067 3.9495-1.5219 6.0023-3.0294a.077.077 0 00.0313-.0552c.5004-5.177-.8382-9.6739-3.5485-13.6604a.061.061 0 00-.0312-.0286zM8.02 15.3312c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9555-2.4189 2.157-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.946 2.419-2.1568 2.419zm7.9748 0c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9554-2.4189 2.1569-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.946 2.419-2.1568 2.419Z"/></svg> },
                      ].map(link => (
                        <a key={link.label} href={link.href} target="_blank" rel="noopener noreferrer"
                          style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', background: link.bg, border: `1px solid ${link.border}`, borderRadius: '6px', textDecoration: 'none', transition: 'all 0.2s', color: '#fff' }}
                          onMouseEnter={(e) => { e.currentTarget.style.opacity = '0.8' }}
                          onMouseLeave={(e) => { e.currentTarget.style.opacity = '1' }}>
                          {link.icon}
                          <span style={{ fontSize: '0.75rem', fontFamily: '"Press Start 2P", cursive, monospace', color: '#fff' }}>{link.label}</span>
                        </a>
                      ))}
                    </div>
                  </div>
              ...
              <button onClick={() => setLeftSidebarOpen(!leftSidebarOpen)} style={{
                ...
                width: '35px', height: '35px', borderRadius: '6px', cursor: 'pointer',
                fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center',
                transition: 'left 0.3s ease', boxShadow: '0 0 10px rgba(16, 185, 129, 0.5)'
              }}>
                {leftSidebarOpen ? '◀' : '▶'}
              </button>
              ...
              <div style={{
                ...
                <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <ThreatGlobe attacks={attacks} activeLayers={activeLayers}
              ...
              <div style={{ position: 'absolute', top: 0, right: 0, width: '300px', bottom: '0', background: 'rgba(0, 10, 0, 0.95)', borderLeft: '1px solid #10b981', zIndex: 90, overflowY: 'auto', scrollbarWidth: 'thin', scrollbarColor: '#10b981 #001100' }}>
                <div style={{ padding: '12px' }}>
                  <div style={{ marginBottom: '15px' }}>
                    <h3 style={{ margin: '0 0 10px 0', color: '#10b981', fontSize: '0.8rem', textTransform: 'uppercase', borderBottom: '1px solid #333', paddingBottom: '6px' }}>🎯 THREAT INTEL</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      {[
                        { label: 'TOTAL ATTACKS', value: stats.totalAttacks, color: '#ef4444', large: true },
                        { label: 'TOP TARGET', value: stats.topTarget, color: '#10b981' },
                        { label: 'THREAT ACTOR', value: stats.topAttacker, color: '#3b82f6' }
                      ].map(s => (
                        <div key={s.label} style={{ background: `${s.color}1a`, padding: '10px', borderRadius: '6px', border: `1px solid ${s.color}` }}>
                          <div style={{ fontSize: '0.65rem', color: s.color, marginBottom: '3px' }}>{s.label}</div>
                          <div style={{ fontSize: s.large ? '1.8rem' : '1.1rem', fontWeight: 'bold', color: '#fff' }}>{s.value}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  <div>
                    <h3 style={{ margin: '0 0 10px 0', color: '#10b981', fontSize: '0.8rem', textTransform: 'uppercase', borderBottom: '1px solid #333', paddingBottom: '6px' }}>📈 VECTORS</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      {['DDoS', 'SQL Injection', 'Ransomware', 'Phishing'].map((type) => {
                        ...
                        return (
                          <div key={type} style={{ marginBottom: '6px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem', color: '#fff', marginBottom: '3px' }}>
                              <span>{type}</span><span>{count}</span>
                            </div>
                            <div style={{ height: '6px', background: '#222', borderRadius: '3px', overflow: 'hidden', border: '1px solid #333' }}>
                              <div style={{ width: `${percentage}%`, height: '100%', background: 'linear-gradient(90deg, #ef4444, #f97316)', transition: 'width 0.5s ease', boxShadow: '0 0 8px rgba(239,68,68,0.5)' }} />
                            </div>
                          </div>
                        )
                      })}
              ...
              <div style={{
                ...
                <div onClick={toggleBottomPanel} style={{
                  ...
                  flexShrink: 0, zIndex: 100, fontSize: '0.8rem', color: '#000', fontWeight: 'bold'
                }}>
                  {bottomPanelOpen ? '▼' : '▲'}
                </div>
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    <div style={{ flex: 1, overflowY: 'auto', padding: '10px', scrollbarWidth: 'thin', scrollbarColor: '#ef4444 #110000' }}>
        ...
        <div ref={cveSectionRef} style={{ width: '100%', minHeight: '100vh', background: 'linear-gradient(180deg, #000 0%, #000d00 100%)', borderTop: '2px solid #10b981', padding: '30px', boxSizing: 'border-box' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px', paddingBottom: '16px', borderBottom: '1px solid #1a3a1a' }}>
            ...
            <div>
              <h2 style={{ margin: 0, color: '#10b981', fontSize: '1.1rem', letterSpacing: '2px', textTransform: 'uppercase' }}>CVE Intelligence Feed</h2>
              <p style={{ margin: 0, color: '#666', fontSize: '0.75rem' }}>Live vulnerability data from NVD National Vulnerability Database</p>
            </div>
            <div style={{ marginLeft: 'auto', background: 'rgba(16,185,129,0.1)', border: '1px solid #10b981', borderRadius: '4px', padding: '4px 10px', fontSize: '0.65rem', color: '#10b981', fontWeight: 'bold', letterSpacing: '1px' }}>LIVE</div>
          </div>
          <CVEFeed />
        </div>
        ...
        <div ref={newsSectionRef} style={{ width: '100%', minHeight: '100vh', background: 'linear-gradient(180deg, #000d00 0%, #00080d 100%)', borderTop: '2px solid #3b82f6', padding: '30px', boxSizing: 'border-box' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px', paddingBottom: '16px', borderBottom: '1px solid #1a2a3a' }}>
            ...
            <div>
              <h2 style={{ margin: 0, color: '#3b82f6', fontSize: '1.1rem', letterSpacing: '2px', textTransform: 'uppercase' }}>Cyber Intelligence News</h2>
              <p style={{ margin: 0, color: '#666', fontSize: '0.75rem' }}>Aggregated security news from trusted sources</p>
            </div>
            <div style={{ marginLeft: 'auto', background: 'rgba(59,130,246,0.1)', border: '1px solid #3b82f6', borderRadius: '4px', padding: '4px 10px', fontSize: '0.65rem', color: '#3b82f6', fontWeight: 'bold', letterSpacing: '1px' }}>AGGREGATED</div>
          </div>
          <LiveNewsFeed />
        </div>
        ...
        <div ref={analyticsSectionRef} style={{ width: '100%', minHeight: '100vh', background: 'linear-gradient(180deg, #00080d 0%, #0d0800 100%)', borderTop: '2px solid #f97316', padding: '30px', boxSizing: 'border-box' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px', paddingBottom: '16px', borderBottom: '1px solid #2a1a0a' }}>
            <span style={{ fontSize: '1.5rem' }}>📊</span>
            <div>
              <h2 style={{ margin: 0, color: '#f97316', fontSize: '1.1rem', letterSpacing: '2px', textTransform: 'uppercase' }}>Threat Analytics</h2>
              <p style={{ margin: 0, color: '#666', fontSize: '0.75rem' }}>Real-time attack pattern analysis and trend visualization</p>
            </div>
          </div>
      ...
      {selectedTool && (
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: '450px', background: 'rgba(0,20,0,0.98)', border: '2px solid #10b981', borderRadius: '12px', padding: '20px', zIndex: 200, boxShadow: '0 0 50px rgba(16,185,129,0.5)', maxHeight: '70vh', overflowY: 'auto' }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', borderBottom: '1px solid #333', paddingBottom: '8px' }}>
              ...
              <button onClick={() => setSelectedTool(null)} style={{ background: 'rgba(239,68,68,0.2)', border: '1px solid #ef4444', color: '#ef4444', width: '28px', height: '28px', borderRadius: '50%', cursor: 'pointer', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}
                onMouseEnter={(e) => { e.currentTarget.style.background = '#ef4444'; e.currentTarget.style.color = '#000' }}
                onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(239,68,68,0.2)'; e.currentTarget.style.color = '#ef4444' }}>×</button>
            </div>
            <p style={{ color: '#ccc', lineHeight: '1.5', marginBottom: '15px', fontSize: '0.9rem' }}>{selectedTool.description}</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '15px' }}>
              {[
                { label: 'License', value: selectedTool.license_type, color: '#10b981' },
                { label: 'Replaces', value: selectedTool.proprietary_alternative, color: '#3b82f6' },
                { label: 'Migration', value: selectedTool.migration_difficulty, color: '#f97316' },
                { label: 'Security', value: selectedTool.signed_releases ? '✅ Signed' : '⚠️ Unsigned', color: selectedTool.signed_releases ? '#10b981' : '#f59e0b' }
              ].map(card => (
                <div key={card.label} style={{ background: `${card.color}1a`, padding: '10px', borderRadius: '6px', border: `1px solid ${card.color}` }}>
                  <div style={{ fontSize: '0.7rem', color: card.color, marginBottom: '3px' }}>{card.label}</div>
                  <div style={{ color: '#fff', fontWeight: 'bold', fontSize: '0.9rem', textTransform: 'capitalize' }}>{card.value}</div>
                </div>
              ))}
            </div>
            {selectedTool.official_link && (
              <a href={selectedTool.official_link} target="_blank" rel="noopener noreferrer"
                style={{ display: 'block', textAlign: 'center', background: 'linear-gradient(135deg, #10b981, #059669)', color: '#000', padding: '10px', borderRadius: '6px', textDecoration: 'none', fontWeight: 'bold', fontSize: '0.9rem', boxShadow: '0 0 15px rgba(16,185,129,0.4)', transition: 'all 0.2s' }}
                onMouseEnter={(e) => { e.currentTarget.style.boxShadow = '0 0 25px rgba(16,185,129,0.7)'; e.currentTarget.style.transform = 'translateY(-2px)' }}
                onMouseLeave={(e) => { e.currentTarget.style.boxShadow = '0 0 15px rgba(16,185,129,0.4)'; e.currentTarget.style.transform = 'translateY(0)' }}>
                Visit Official Site ↗
              </a>
            )}
          </div>
